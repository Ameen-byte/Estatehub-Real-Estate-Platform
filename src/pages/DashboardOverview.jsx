...
const emptyForm = { title: '', location: '', price: '', tag: 'New listing', status: 'Active', image: 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&w=800&q=80', description: '' }
...
function DashboardOverview() {
  ...
  const stats = [{ label: 'Total properties', value: properties.length, change: 'Saved across devices', icon: 'H' }, { label: 'Active listings', value: properties.filter((property) => property.status === 'Active').length, change: 'Live in your portfolio', icon: '●' }, { label: 'API records', value: properties.filter((property) => property.id.startsWith('api-')).length, change: loading ? 'Loading live data' : 'Fetched with useEffect', icon: '↗' }]
