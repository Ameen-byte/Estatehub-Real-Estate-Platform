...
function Login({ onLogin }) {
  ...
  const handleSubmit = (event) => { event.preventDefault(); const next = validate(); setErrors(next); if (Object.keys(next).length) return; setStatus('loading'); setMessage('Checking your EstateHub account...'); window.setTimeout(() => { if (email === 'admin@estatehub.com' && password === 'EstateHub@123') { onLogin({ email, name: 'Ameen Siddiqui' }); setStatus('success'); setMessage('Welcome back. Redirecting to your dashboard...'); window.setTimeout(() => navigate(location.state?.from?.pathname || '/dashboard'), 500) } else { setStatus('failure'); setMessage('Invalid credentials. Try admin@estatehub.com and EstateHub@123 for the demo.'); } }, 2000) }
